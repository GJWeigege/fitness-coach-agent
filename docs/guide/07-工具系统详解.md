...
**与 knowledge_prefetch 的区别**：`knowledge_prefetch_node` 在 **unknown** 意图路径上预取 RAG（并可选写 `graph_context`），不经过工具注册表；sub_agent 路径上的 `knowledge_search` 是 LLM **主动决策**调用，两者触发时机不同。
...
| 工具 | training | nutrition | safety | profile | unknown/chitchat |
|------|:--------:|:---------:|:------:|:-------:|:----------------:|
| knowledge_search | ✓ | ✓ | ✓ | △ | unknown→prefetch；chitchat 通常不调 |
...
（✓ 高频；△ 偶发；— 通常不触发；unknown 经 prefetch 预检索，chitchat 不经 sub_agent 工具环）
