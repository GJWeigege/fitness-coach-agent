...
3. **回合后**：`_complete_turn_events` → 摘要（older）→ 写 assistant → `finalize_run` → `done`（详见 [AGENT_SSE](../reference/AGENT_SSE.md) 七步契约）
...
```text
上传/seed
  → ingest_service（Markdown 按 ## 分块；节内 400 字/重叠 60 + embedding）
  → knowledge_chunks (vector 1024 维)

提问
  → knowledge_search / rag_service（混合检索 → rerank → MMR）
  → unknown 意图：prefetch 内 `resolve_graph_context_for_rag`
  → sub_agent 路径：可选 `graph_lookup` 工具（非 knowledge_search 自动 enrich）
...
erDiagram
    ...
    chat_messages }o--o| agent_runs : "agent_run_id（逻辑关联，非 FK）"
...
| 表 | 关键字段 | 用途 |
|----|----------|------|
| `chat_sessions` | `summary`, `turn_count` | 滚动摘要与触发计数 |
| `chat_messages` | `retrieved_chunks`, `agent_steps`, `agent_run_id` | 助手消息附 citations 与 step 快照 |
| `agent_runs` | `intent`, `status`, `memory_compacted` | 单轮编排观测 |
| `agent_steps` | `phase`, `payload` | planning / tool / finalize JSON |
| `llm_calls` | `purpose`, `prompt_tokens` | 按 router/planner/sub_agent 等分 purpose |

### 6.2 知识库与 Graph

| 表 | 关键字段 | 用途 |
|----|----------|------|
| `knowledge_documents` | `status`, `content_hash` | 上传文档生命周期 |
| `knowledge_chunks` | `embedding Vector(1024)`, `chunk_index` | pgvector 检索单元 |
| `graph_entities` | `entity_type`, `name`, `embedding` | 实体 linking |
| `graph_edges` | `relation_type`, `source_id`, `target_id` | 1-hop 上下文 |
| `graph_entity_links` | `chunk_id`, `entity_id`, `confidence` | RAG 动态关联 |
| `retrieval_logs` | `query`, `results` JSONB | 检索 debug |

### 6.3 JSONB 字段约定

- `chat_messages.retrieved_chunks`：`{"items": [citation, ...]}`，与 SSE `session.citations` 同源
- `chat_messages.agent_steps`：`{"items": [{phase, summary, ...}]}`，来自 ChatService 收集的 SSE step
- `knowledge_documents` / benchmark 相关字段见 [09-评测与观测](./09-评测与观测.md)

---

## 7. Observability 与 Benchmark
