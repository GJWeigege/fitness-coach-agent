...
| 3 | `services/chat_service.py` | 六步契约、`_complete_turn_events` |
...
| 子步 | 动作 |
|------|------|
| 3a | INSERT `ChatMessage` assistant，`retrieved_chunks={"items": citations}` |
| 3b | `memory_service.maybe_update_summary` |
| 3c | 可选 yield `step` phase=`memory_summary` |
| 3d | `orchestrator.finalize_run` → 更新 `AgentRun`、合并 step payload |
| 3e | yield `done` 含 `assistant_message_id` |
...
- [../specs/COACH_AGENT_REDESIGN.md](../specs/COACH_AGENT_REDESIGN.md) — §3.5 ChatService 六步
